...
import layout from '../templates/components/ed-temp-placeholder-insert-after';
...
export default EdBasicPlaceholderComponent.extend(MouseInMixin, {
	...
	mouseUp(){
		if (this.get("edState.isModeDrag")){
			...
			const idx = this.get("domElement.parent.children").indexOf(this.get("domElement"));

			this.set("edState.moveElement.parent", this.get("domElement.parent"));
			this.get("domElement.parent.children").insertAt(idx + 1, this.get("edState.moveElement"));
